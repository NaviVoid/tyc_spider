 ...
 * @param save_point 上次最后成功爬取的公司名字
 * @param NAMES 公司名字
 */
const start_fetch = async (
  ...
  log.info(`[start_fetch] token: ${token}, save point: "${save_point}"`);
  ...
  // 跳过 save_point
  let names = NAMES;
  if (save_point && NAMES.indexOf(save_point) !== -1) {
    names = NAMES.slice(NAMES.indexOf(save_point));
  }
